...
// Fetch warehouse options from DB
$warehouses = [];
$w_result = mysqli_query($conn, "SELECT * FROM warehouses ORDER BY name ASC");
if($w_result){
...
if(isset($_POST['name'], $_POST['sku'], $_POST['category'], $_POST['unit'], $_POST['stock'], $_POST['warehouse'])){
    ...
    if(mysqli_num_rows($check_sku) > 0){
        ...
        $sql = "INSERT INTO products (name, sku, category, unit, stock, warehouse, created_at)
                VALUES ('$name','$sku','$category','$unit',$stock,'$warehouse',NOW())";
...
<head>
    ...
    <style>
        ...
        :root { --primary: #6366f1; --primary-dark: #4f46e5; --bg: #f8fafc; --card-bg: #ffffff; --text-main: #1e293b; --text-muted: #64748b; --sidebar: #0f172a; --danger: #ef4444; --success: #10b981; }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-card { background: var(--card-bg); width: 100%; max-width: 500px; padding: 2.5rem; border-radius: 20px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.1); border: 1px solid #e2e8f0; animation: scaleUp 0.4s ease-out; }
        ...
        input, select { width: 100%; padding: 12px 12px 12px 40px; border: 1px solid #e2e8f0; border-radius: 10px; font-size: 0.95rem; transition: all 0.3s; background: #fcfcfd; }
        input:focus, select:focus { outline: none; border-color: var(--primary); background: #fff; box-shadow: 0 0 0 4px rgba(99,102,241,0.1); }
        ...
        button { width: 100%; padding: 14px; background: var(--primary); color: white; border: none; border-radius: 10px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: 0.3s; margin-top: 1rem; }
            ...
            <div class="input-group">
                ...
                <select name="warehouse" required>
                    ...
                    if(!empty($warehouses)){
                        ...
                        $static_warehouses = ["Central Warehouse","North Warehouse","South Warehouse","East Warehouse","West Warehouse"];
                        foreach($static_warehouses as $w){
                            echo "<option value=\"$w\">$w</option>";
                        }
                    }
                    ?>
